...
| **Buy Units from the wallet** | Fund the Units balance over x402, paying in Monetize Credits or in on-chain Base USDC. One command, no browser, no card | Use when the balance is low or negative and the wallet holds either credits or USDC | `fluxa-wallet market model topup --bundle starter`, `--usdc` to pay on-chain (confirm the spend first); procedure: https://agentmarket.fluxapay.xyz/marketplace/models/agent-topup.md |
| **Buy Units with a card** | Fund the Units balance with the user's linked card, through a merchant-scoped `CARD_USD` mandate | Use when the wallet has neither credits nor USDC. **Follow the published procedure, do not improvise** | Read https://agentmarket.fluxapay.xyz/marketplace/models/topup.md and follow it exactly |
