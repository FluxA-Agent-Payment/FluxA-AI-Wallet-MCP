...
| **Buy Units with USDC** | Fund the Units balance with on-chain USDC on Base instead of credits | Use when there are no credits to spend. Cards are not a rail for Units topups | `fluxa-wallet market model topup --usdc` (confirm the spend first); full procedure: https://agentmarket.fluxapay.xyz/marketplace/models/topup.md |
